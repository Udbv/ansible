...
class CallbackModule(CallbackBase):
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        if result._task.loop and 'results' in result._result:
            ...
            if delegated_vars:
                self._display.display("##[error]fatal: [%s -> %s]: FAILED! => %s" % (result._host.get_name(), delegated_vars['ansible_host'],
                                                                            self._dump_results(result._result)),
                                      color=C.COLOR_ERROR, stderr=self.display_failed_stderr)
            else:
                self._display.display("##[error]fatal: [%s]: FAILED! => %s" % (result._host.get_name(), self._dump_results(result._result)),
                                      color=C.COLOR_ERROR, stderr=self.display_failed_stderr)
    ...
    def v2_runner_on_ok(self, result):
        ...
        if isinstance(result._task, TaskInclude):
            ...
            if delegated_vars:
                msg = "##[warning]changed: [%s -> %s]" % (result._host.get_name(), delegated_vars['ansible_host'])
            else:
                msg = "##[warning]changed: [%s]" % result._host.get_name()
            ...
            if delegated_vars:
                msg = "##[section]ok: [%s -> %s]" % (result._host.get_name(), delegated_vars['ansible_host'])
            else:
                msg = "##[section]ok: [%s]" % result._host.get_name()
            color = C.COLOR_OK
    ...
    def v2_runner_on_skipped(self, result):
        ...
        if self.display_skipped_hosts:
            ...
            if result._task.loop and 'results' in result._result:
                ...
                msg = "##[debug]skipping: [%s]" % result._host.get_name()
                if self._run_is_verbose(result):
                    msg += " => %s" % self._dump_results(result._result)
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        delegated_vars = result._result.get('_ansible_delegated_vars', None)
        if delegated_vars:
            msg = "##[error]fatal: [%s -> %s]: UNREACHABLE! => %s" % (result._host.get_name(), delegated_vars['ansible_host'], self._dump_results(result._result))
        else:
            msg = "##[error]fatal: [%s]: UNREACHABLE! => %s" % (result._host.get_name(), self._dump_results(result._result))
        self._display.display(msg, color=C.COLOR_UNREACHABLE, stderr=self.display_failed_stderr)

    def v2_playbook_on_no_hosts_matched(self):
        self._display.display("##[debug]skipping: no hosts matched", color=C.COLOR_SKIP)

    def v2_playbook_on_no_hosts_remaining(self):
        self._display.banner("NO MORE HOSTS LEFT")

    def v2_playbook_on_task_start(self, task, is_conditional):
        self._task_start(task, prefix='##[command]TASK')
    ...
    def _print_task_banner(self, task):
        ...
        prefix = self._task_type_cache.get(task._uuid, '##[command]TASK')
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        if play.check_mode and self.check_mode_markers:
            ...
            checkmsg = ""
        if not name:
            msg = u"##[command]PLAY%s" % checkmsg
        else:
            msg = u"##[command]PLAY [%s]%s" % (name, checkmsg)
    ...
    def v2_runner_item_on_skipped(self, result):
        if self.display_skipped_hosts:
            ...
            self._clean_results(result._result, result._task.action)
            msg = "##[debug]skipping: [%s] => (item=%s) " % (result._host.get_name(), self._get_item_label(result._result))
            if self._run_is_verbose(result):
                msg += " => %s" % self._dump_results(result._result)
            self._display.display(msg, color=C.COLOR_SKIP)

    def v2_playbook_on_include(self, included_file):
        msg = '##[debug]included: %s for %s' % (included_file._filename, ", ".join([h.name for h in included_file._hosts]))
    ...
    def v2_playbook_on_stats(self, stats):
        self._display.banner("##[command]PLAY RECAP")
    ...
    def v2_playbook_on_start(self, playbook):
        if self._display.verbosity > 1:
            ...
            self._display.banner("##[command]PLAYBOOK: %s" % basename(playbook._file_name))
